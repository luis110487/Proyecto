...
def venta_create(request):
    if request.method == 'POST':
        # Si es para agregar producto a la lista temporal
        if 'agregar_producto' in request.POST:
            producto_id = request.POST.get('producto')
            cantidad = request.POST.get('cantidad', 1)
            ...
            if producto_id and cantidad and precio_venta:
                ...
                try:
                    ...
                    if cantidad_int > producto.stock:
                        ...
                        messages.success(request, f'Producto "{producto.nombre}" agregado a la venta')
        ...
        # Si es para guardar la venta completa
        elif 'guardar_venta' in request.POST:
            form = VentaForm(request.POST)
            productos_venta = request.session.get('productos_venta', [])
            
            if form.is_valid() and productos_venta:
                try:
                    venta = form.save()
